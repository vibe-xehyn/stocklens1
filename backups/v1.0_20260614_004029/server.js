...
async function fetchMacroData() {
  ...
  // 4. 야후 파이낸스 국채 금리 지표 (야후 v8 차단 시 상주형 Python 워커 yfRun 백업 가동)
  const yahooJobs = [
    ['^TNX', 'us10y'],
    ['TMUBMUSD03Y', 'us3y'],
    ['^FVX', 'us5y'],
...
if len(h) >= 1:
    ...
    prev = float(h['Close'].iloc[-2]) if len(h) >= 2 else price
    chg = price - prev
    print(json.dumps({'price': price, 'change': chg}))
else:
    print(json.dumps({}))
`;
        const q = await yfRun(py);
        if (q && q.price) {
          result[key] = { value: q.price, change: Math.round(q.change * 100) / 100 };
        }
      } catch (err) {
        console.error(`[fetchMacroData] yfinance python fallback failed for ${sym}:`, err.message);
      }
    }
  }));
