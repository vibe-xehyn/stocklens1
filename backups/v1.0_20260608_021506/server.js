  ...
  try {
    const data = await cached('rates', 300_000, async () => {
      try {
        ...
        console.warn(`[api/rates] Primary NAVER fetch failed or partial: ${err.message}. Falling back to Yahoo/Stooq...`);
      }
        ...
        try {
          ...
          try { q = await yahooQuote(sym); }
            ...
            try { q = await stooqQuote(sym); }
              ...
              // Yahoo and Stooq both failed, fallback to NAVER individually
              if (key === 'usdkrw') {
                const d = await fetchJSON('https://api.stock.naver.com/marketindex/exchange/FX_USDKRW', NAVER_REF);
                const info = d.exchangeInfo;
                q = {
          ...
          console.error(`[api/rates] error fetching key ${key}:`, err.message);
