...
async function _pyExec(script, timeout = 15000) {
  // 가장 한가한 워커 선택 (pending 수 기준)
  const w = _pyPool.reduce((a, b) => (a.load() <= b.load() ? a : b));
  return w.exec(script, timeout);
}

function _pyExecMedium(script) { return _pyExec(script, 15000); } // 15초 (단일 종목 API 등)
...
app.get('/api/chart', async (req, res) => {
  ...
  await serveSWR(res, `c:${symbol}:${range}`, 300_000, () => fetchChartData(symbol, yfRange, interval, market));
});
