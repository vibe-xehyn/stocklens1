...
app.get('/api/index-chart', async (req, res) => {
  ...
  try {
    const data = await cached(`ic:${symbol}:${range}`, 300_000, async () => {
      ...
      const usMap  = { 'S&P 500': '.INX', NASDAQ: '.IXIC', DOW: '.DJI', DXY: '.DXY' };
      ...
      try {
        ...
        else if (symbol === 'USD/KRW') rows = await naverExchangeChart('FX_USDKRW', range);
      ...
      if (!rows.length) {
        try {
          const yMap = { KOSPI: '^KS11', KOSDAQ: '^KQ11', 'S&P 500': '^GSPC', NASDAQ: '^IXIC', DOW: '^DJI', 'USD/KRW': 'KRW=X', 'DXY': 'DX-Y.NYB', 'EUR/USD': 'EURUSD=X', 'USD/JPY': 'JPY=X' };
