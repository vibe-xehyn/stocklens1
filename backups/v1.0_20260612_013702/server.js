...
if len(h) >= 1:
    ...
    print(json.dumps({'price': price, 'changePct': pct}))
else:
    print(json.dumps({}))
`;
    const q = await yfRun(py);
    if (q && q.price) {
      return { name, value: q.price, change: Math.round(q.changePct * 100) / 100 };
    }
  } catch (err) {
    console.error(`[yfIndex] yfinance python fallback failed for ${sym}:`, err.message);
  }
