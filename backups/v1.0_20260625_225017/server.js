...
// 세션을 파일 데이터베이스에 저장하여 서버 재시작 시에도 유지되도록 함
const sessionsMap = readJSONFile(SESSIONS_FILE);
const sessions = {
  get(token) { return sessionsMap[token]; },
  set(token, user) {
    sessionsMap[token] = user;
    writeJSONFile(SESSIONS_FILE, sessionsMap);
  },
  delete(token) {
...
function requireAdmin(req, res, next) {
  const adminToken = process.env.ADMIN_TOKEN || 'stocklens_admin_secret_key_2026';
...
// Global API Protection Middleware: All /api/ routes require login except auth/config routes
app.use((req, res, next) => {
  ...
  if (req.path.startsWith('/api/') && !publicPaths.includes(req.path)) {
    // If it's an admin endpoint, let the admin middleware handle it later
...
// ── 보안 헤더 미들웨어 (Clickjacking, MIME Sniffing, CSP 등 방지) ──────────────
app.use((req, res, next) => {
  res.setHeader('X-Frame-Options', 'DENY');
  res.setHeader('X-Content-Type-Options', 'nosniff');
  res.setHeader('X-XSS-Protection', '1; mode=block');
  res.setHeader('Referrer-Policy', 'strict-origin-when-cross-origin');
  res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' https:; frame-ancestors 'none';");
  next();
});
...
app.get('/api/auth/config', (req, res) => {
  ...
  res.json({
    googleClientId: process.env.GOOGLE_CLIENT_ID || settings.googleClientId || null
  });
});
...
app.post('/api/auth/social', (req, res) => {
  const { email, name, provider, credential } = req.body;
  ...
  if (provider === 'google' && credential) {
    try {
      ...
      if (parts.length === 3) {
        ...
        return res.status(400).json({ error: '인증 토큰 형식이 유효하지 않습니다.' });
      }
    } catch (e) {
      return res.status(400).json({ error: '구글 토큰 검증에 실패했습니다.' });
    }
  }
...
app.post('/api/auth/firebase', (req, res) => {
  ...
  if (!idToken) {
    return res.status(400).json({ error: '파이어베이스 토큰이 누락되었습니다.' });
  }
  ...
  try {
    ...
    return res.status(400).json({ error: '토큰 유효성 검증에 실패했습니다.' });
  }
});
...
app.post('/api/user/delete-account', requireAuth, (req, res) => {
  try {
    ...
    // 1. users.json에서 사용자 계정 정보 완전히 삭제
    const users = readJSONFile(USERS_FILE);
    if (userEmail) {
      const normalizedEmail = userEmail.trim().toLowerCase();
    ...
    // 2. userdata.json에서 해당 사용자의 관심목록, 포트폴리오, 알림 등 전용 데이터 완전 삭제
...
function _dayRange(range) {
  const days = { '1d':2, '1w':10, '3m':100, '1y':380, '5y':1825, 'max':7300 }[range] || 45;
...
async function naverExchangeChart(code, range = '1mo') {
  const limitMap = { '1wk': 7, '1mo': 22, '3mo': 66, '6mo': 130, '1y': 260, '5y': 1300, 'max': 2600 };
  const limit = limitMap[range] || 260;
  
  if (limit <= 60) {
    const d = await fetchJSON(`https://api.stock.naver.com/marketindex/exchange/${code}/prices?page=1&pageSize=${limit}`, NAVER_REF);
    if (!Array.isArray(d) || !d.length) throw new Error('NAVER exchange chart: no data');
    return d.map(r => ({
      date: r.localTradedAt,
      close: parseFloat(r.closePrice.replace(/,/g, ''))
    })).reverse();
  } else {
    const pagesCount = Math.min(10, Math.ceil(limit / 60));
    const pages = Array.from({ length: pagesCount }, (_, i) => i + 1);
    const results = await Promise.allSettled(pages.map(page =>
      fetchJSON(`https://api.stock.naver.com/marketindex/exchange/${code}/prices?page=${page}&pageSize=60`, NAVER_REF)
    ));
    ...
    if (!allRows.length) throw new Error('NAVER exchange chart: no data');
    
    const unique = [];
    const seen = new Set();
    for (const r of allRows) {
      if (!seen.has(r.localTradedAt)) {
        seen.add(r.localTradedAt);
        unique.push({
          date: r.localTradedAt,
          close: parseFloat(r.closePrice.replace(/,/g, ''))
        });
...
async function yahooQuote(symbol) {
  ...
  if (price == null) throw new Error('Yahoo: price is null');
...
let _yfActive = 0;
const _yfQueue = [];
const YF_CONCURRENCY = 1;
const YF_GAP = 300; // ms between starts

function yfRun(script, timeout = 6000) {
  // UNDEFINED 심볼을 포함한 스크립트 즉시 거부 (undefined ticker 방어)
  if (/Ticker\(['"]UNDEFINED['"]\)|Ticker\(['"]undefined['"]\)|download\([^)]*UNDEFINED/i.test(script)) {
    return Promise.reject(new Error('yfRun: UNDEFINED symbol rejected'));
  }
  return new Promise((resolve, reject) => {
    _yfQueue.push({ script, resolve, reject, timeout });
    _yfDrain();
  });
}
...
function _yfDrain() {
  while (_yfActive < YF_CONCURRENCY && _yfQueue.length > 0) {
    const { script, resolve, reject, timeout } = _yfQueue.shift();
    _yfActive++;
    _pyExec(script, timeout)
      .then(resolve, reject)
      .finally(() => {
        _yfActive--;
...
class PyWorker {
  ...
  spawn() {
    ...
    this.proc.on('exit', code => {
      console.error(`  ⚠ Python 워커 #${this.id} 종료 (code=${code}). 500ms 후 재시작.`);
  ...
  async exec(script, timeout) {
    if (!this.ready || !this.proc) await this.readyPromise;
    const id = String(++_pyRid);
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id);
        console.error(`  ⚠ Python 워커 #${this.id} 작업 시간 초과 (${timeout}ms). 프로세스 종료 및 재시작.`);
        if (this.proc) {
          try {
            this.proc.kill('SIGKILL');
          } catch (e) {
            console.error(`Failed to kill python worker: ${e.message}`);
          }
        }
...
async function _pyExec(script, timeout = 8000) {
  // 가장 한가한 워커 선택 (pending 수 기준)
  const w = _pyPool.reduce((a, b) => (a.load() <= b.load() ? a : b));
  return w.exec(script, timeout);
}

function _pyExecMedium(script) { return _pyExec(script, 8000); } // 8초 (단일 종목 API 등)
...
async function krChart(ticker, range, interval) {
  try {
    ...
    console.warn(`[krChart] Yahoo failed: ${e.message}. Trying NAVER...`);
    try {
      return await naverKrItemChart(ticker, range);
    } catch (err) {
      console.error(`[krChart] NAVER failed: ${err.message}`);
      throw err;
    }
  }
}
...
async function usChart(ticker, range, interval) {
  try {
    ...
    console.warn(`[usChart] Yahoo failed: ${e.message}. Trying NAVER...`);
    try {
      return await naverUsChart(ticker, range);
    } catch (err) {
      console.error(`[usChart] NAVER failed: ${err.message}`);
      throw err;
    }
  }
}
...
app.get('/api/sidebar', async (req, res) => {
  ...
  if (!symbol || symbol === 'undefined') return res.status(400).json({ error: 'symbol required' });
  try {
  ...
  try {
    const data = await cached(`ic:${symbol}:${range}`, 300_000, async () => {
      ...
      try {
        ...
        else if (symbol === 'USD/KRW' || symbol === 'usd/krw') rows = await naverExchangeChart('FX_USDKRW', range);
        else if (symbol === 'EUR/USD') {
          const [eur, usd] = await Promise.all([
            naverExchangeChart('FX_EURKRW', range),
            naverExchangeChart('FX_USDKRW', range)
          ]);
          const usdMap = new Map(usd.map(x => [x.date, x.close]));
          rows = eur.map(x => {
            const uVal = usdMap.get(x.date);
            return uVal ? { date: x.date, close: x.close / uVal } : null;
          }).filter(Boolean);
        } else if (symbol === 'USD/JPY') {
          const [usd, jpy] = await Promise.all([
            naverExchangeChart('FX_USDKRW', range),
            naverExchangeChart('FX_JPYKRW', range)
          ]);
          const jpyMap = new Map(jpy.map(x => [x.date, x.close]));
          rows = usd.map(x => {
            const jVal = jpyMap.get(x.date);
            return jVal ? { date: x.date, close: (x.close / jVal) * 100 } : null;
          }).filter(Boolean);
        }
      } catch (err) {
        console.error(`[index-chart] NAVER failed for ${symbol}:`, err.message);
      }
      // Fallback: Yahoo v8
      if (!rows.length) {
        try {
          const yMap = {
            ...
            'USD/KRW': 'KRW=X', 'DXY': 'DX-Y.NYB', 'EUR/USD': 'EURUSD=X', 'USD/JPY': 'JPY=X',
            'VIX': '^VIX', 'vix': '^VIX', '^VIX': '^VIX',
            'Gold': 'GC=F', 'gold': 'GC=F', '금': 'GC=F',
            'Oil': 'CL=F', 'oil': 'CL=F', 'WTI 원유': 'CL=F',
          ...
          console.warn(`[index-chart] Yahoo failed for ${symbol}: ${err.message}`);
          rows = [];
        }
      }
...
(function loadInitCaches() {
  try {
    if (existsSync(RATES_CACHE_FILE)) {
      ...
      if (c.data) setC('rates', c.data, 300_000);
    }
  } catch {}
  try {
...
async function fetchRatesData() {
  try {
    const [usdkrwRes, jpykrwRes, eurkrwRes, dxyRes] = await Promise.all([
      fetchJSON('https://api.stock.naver.com/marketindex/exchange/FX_USDKRW', NAVER_REF),
      fetchJSON('https://api.stock.naver.com/marketindex/exchange/FX_JPYKRW', NAVER_REF),
      fetchJSON('https://api.stock.naver.com/marketindex/exchange/FX_EURKRW', NAVER_REF),
      fetchJSON('https://polling.finance.naver.com/api/realtime/worldstock/index/.DXY', NAVER_REF).catch(() => null)
    ]);

    const rates = {};

    const usdkrw = parseFloat(usdkrwRes.exchangeInfo.closePrice.replace(/,/g, ''));
    const usdkrwChg = parseFloat(usdkrwRes.exchangeInfo.fluctuations) * (usdkrwRes.exchangeInfo.fluctuationsType.name === 'FALLING' ? -1 : 1);
    const usdkrwChgPct = parseFloat(usdkrwRes.exchangeInfo.fluctuationsRatio) * (usdkrwRes.exchangeInfo.fluctuationsType.name === 'FALLING' ? -1 : 1);
    rates['usdkrw'] = { value: usdkrw, change: Math.round(usdkrwChgPct * 100) / 100 };

    const jpykrw = parseFloat(jpykrwRes.exchangeInfo.closePrice.replace(/,/g, ''));
    const jpykrwChg = parseFloat(jpykrwRes.exchangeInfo.fluctuations) * (jpykrwRes.exchangeInfo.fluctuationsType.name === 'FALLING' ? -1 : 1);

    const eurkrw = parseFloat(eurkrwRes.exchangeInfo.closePrice.replace(/,/g, ''));
    const eurkrwChg = parseFloat(eurkrwRes.exchangeInfo.fluctuations) * (eurkrwRes.exchangeInfo.fluctuationsType.name === 'FALLING' ? -1 : 1);

    const usdjpy = (usdkrw / jpykrw) * 100;
    const usdkrwPrev = usdkrw - usdkrwChg;
    const jpykrwPrev = jpykrw - jpykrwChg;
    const usdjpyPrev = (usdkrwPrev / jpykrwPrev) * 100;
    const usdjpyChgPct = ((usdjpy - usdjpyPrev) / usdjpyPrev) * 100;
    rates['usdjpy'] = { value: Math.round(usdjpy * 10000) / 10000, change: Math.round(usdjpyChgPct * 100) / 100 };

    const eurusd = eurkrw / usdkrw;
    const eurkrwPrev = eurkrw - eurkrwChg;
    const eurusdPrev = eurkrwPrev / usdkrwPrev;
    const eurusdChgPct = ((eurusd - eurusdPrev) / eurusdPrev) * 100;
    rates['eurusd'] = { value: Math.round(eurusd * 10000) / 10000, change: Math.round(eurusdChgPct * 100) / 100 };

    if (dxyRes && dxyRes.datas && dxyRes.datas[0]) {
      const dxyInfo = dxyRes.datas[0];
      const dxyPrice = parseFloat(dxyInfo.closePrice.replace(/,/g, ''));
      const dxyChgPct = parseFloat(dxyInfo.fluctuationsRatio) * (dxyInfo.compareToPreviousPrice.name === 'FALLING' ? -1 : 1);
      rates['dxy'] = { value: dxyPrice, change: Math.round(dxyChgPct * 100) / 100 };
    } else {
      throw new Error('DXY failed on NAVER');
    }

    if (rates.usdkrw && rates.usdjpy && rates.eurusd && rates.dxy) {
      return rates;
    }
  } catch (err) {
    console.warn(`[fetchRatesData] Primary NAVER fetch failed or partial: ${err.message}. Falling back to Yahoo/Stooq...`);
  }
  ...
  await Promise.allSettled(yPairs.map(async ([sym, key]) => {
    try {
      let q;
      try { q = await yahooQuote(sym); }
      catch {
        try { q = await stooqQuote(sym); }
        catch (e) {
          if (key === 'usdkrw') {
            const d = await fetchJSON('https://api.stock.naver.com/marketindex/exchange/FX_USDKRW', NAVER_REF);
            const info = d.exchangeInfo;
            q = {
              price: parseFloat(info.closePrice.replace(/,/g, '')),
              changePct: parseFloat(info.fluctuationsRatio) * (info.fluctuationsType.name === 'FALLING' ? -1 : 1)
            };
          } else if (key === 'dxy') {
            const d = await fetchJSON('https://polling.finance.naver.com/api/realtime/worldstock/index/.DXY', NAVER_REF);
            const info = d.datas?.[0];
            q = {
              price: parseFloat(info.closePrice.replace(/,/g, '')),
              changePct: parseFloat(info.fluctuationsRatio) * (info.compareToPreviousPrice.name === 'FALLING' ? -1 : 1)
            };
          } else if (key === 'usdjpy') {
            ...
            const usdkrwChg = parseFloat(d1.exchangeInfo.fluctuations) * (d1.exchangeInfo.fluctuationsType.name === 'FALLING' ? -1 : 1);
            const jpykrwChg = parseFloat(d2.exchangeInfo.fluctuations) * (d2.exchangeInfo.fluctuationsType.name === 'FALLING' ? -1 : 1);
            const usdkrwPrev = usdkrw - usdkrwChg;
            const jpykrwPrev = jpykrw - jpykrwChg;
            const usdjpyPrev = (usdkrwPrev / jpykrwPrev) * 100;
            const chgPct = ((usdjpy - usdjpyPrev) / usdjpyPrev) * 100;

            q = { price: usdjpy, changePct: chgPct };
          } else if (key === 'eurusd') {
            const [d1, d2] = await Promise.all([
              fetchJSON('https://api.stock.naver.com/marketindex/exchange/FX_EURKRW', NAVER_REF),
              fetchJSON('https://api.stock.naver.com/marketindex/exchange/FX_USDKRW', NAVER_REF)
            ]);
            const eurkrw = parseFloat(d1.exchangeInfo.closePrice.replace(/,/g, ''));
            const usdkrw = parseFloat(d2.exchangeInfo.closePrice.replace(/,/g, ''));
            const eurusd = eurkrw / usdkrw;

            const eurkrwChg = parseFloat(d1.exchangeInfo.fluctuations) * (d1.exchangeInfo.fluctuationsType.name === 'FALLING' ? -1 : 1);
            const usdkrwChg = parseFloat(d2.exchangeInfo.fluctuations) * (d2.exchangeInfo.fluctuationsType.name === 'FALLING' ? -1 : 1);
            const eurkrwPrev = eurkrw - eurkrwChg;
            const usdkrwPrev = usdkrw - usdkrwChg;
            const eurusdPrev = eurkrwPrev / usdkrwPrev;
            const chgPct = ((eurusd - eurusdPrev) / eurusdPrev) * 100;

            q = { price: eurusd, changePct: chgPct };
          } else {
            throw new Error('Fallback not mapped');
          }
        }
      }
      rates[key] = { value: q.price, change: Math.round(q.changePct * 100) / 100 };
    } catch (err) {
      console.error(`[fetchRatesData] error fetching key ${key}:`, err.message);
      rates[key] = { value: null, change: null };
    }
  }));
  return rates;
}
...
function aggregateToYearly(rows) {
  const groups = {};
  for (const r of rows) {
    if (!r.date) continue;
  ...
  for (const y of years) {
    ...
    const high = Math.max(...group.map(g => g.high ?? g.close));
    const low = Math.min(...group.map(g => g.low ?? g.close));
    const volume = group.reduce((sum, g) => sum + (g.volume ?? 0), 0);
    yearlyRows.push({
      date: `${y}-01-01T00:00:00.000Z`,
...
async function fetchChartData(symbol, range, interval, market) {
  ...
  const actualInterval = isYearly ? '1mo' : interval;
  const raw = market==='kr' ? await krChart(symbol,range,actualInterval) : await usChart(symbol,range,actualInterval);
  
  const processedRows = isYearly ? aggregateToYearly(raw) : raw;

  let fmtOpts = { month:'short', day:'numeric' };
  if (isYearly) fmtOpts = { year: 'numeric' };
  else if (range === '1d') fmtOpts = { hour: '2-digit', minute: '2-digit' };
  ...
  return {
    labels: processedRows.map(d => new Date(d.date).toLocaleDateString('ko', fmtOpts)),
    data:   processedRows.map(d => d.close),
    // OHLCV 지원 (캔들 차트용)
    ohlcv:  processedRows.map(d => ({
      time:   toUnix(d.date),
      open:   d.open  ?? d.close,
      high:   d.high  ?? d.close,
      low:    d.low   ?? d.close,
...
app.get('/api/chart', async (req, res) => {
  const { symbol, range='1y', interval: reqInterval, market } = req.query;
  if (!symbol || symbol === 'UNDEFINED' || symbol === 'undefined' || symbol === 'null') return res.status(400).json({ error: 'symbol required' });
  
  let interval = reqInterval || '1d';
  let yfRange = range;
  
  if (!reqInterval) {
    // yfinance range mapping compatibility
    if (range === '1w') {
      yfRange = '5d'; // yfinance supports 5d
      interval = '15m'; // 15분봉
    } else if (range === '3m') {
      yfRange = '3mo';
      interval = '1d';
  ...
  await serveSWR(res, `c:${symbol}:${range}:${interval}`, 300_000, () => fetchChartData(symbol, yfRange, interval, market));
});
...
async function yfIndex(sym, name) {
  if (sym === 'KRW=X') {
    const ratesCache = getC('rates');
    if (ratesCache && ratesCache.usdkrw && ratesCache.usdkrw.value !== null) {
      return { name, value: ratesCache.usdkrw.value, change: ratesCache.usdkrw.change };
    }
    try {
      const d = await fetchJSON('https://api.stock.naver.com/marketindex/exchange/FX_USDKRW', NAVER_REF);
      const info = d.exchangeInfo;
      const price = parseFloat(info.closePrice.replace(/,/g, ''));
      const change = parseFloat(info.fluctuationsRatio) * (info.fluctuationsType.name === 'FALLING' ? -1 : 1);
      return { name, value: price, change: Math.round(change * 100) / 100 };
    } catch (e) {
      console.error(`[yfIndex] failed to fetch KRW=X from NAVER:`, e.message);
    }
  }
  // 1차: NAVER (Railway/OCI에서 극도 안전)
...
app.get('/api/rates', async (_, res) => {
  try {
    const data = await cached('rates', 300_000, () => fetchRatesData());
    res.json(data);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/news', async (req, res) => {
  const { symbol, market } = req.query;
  return serveSWR(res, `n:${symbol}`, 3600_000, () => getNews(symbol, market==='kr'));
});
...
`;
  return _pyExecMedium(py);
}

app.get('/api/flow', async (req, res) => {
  const { symbol, market } = req.query;
  if (!symbol || symbol === 'undefined') return res.status(400).json({ error: 'symbol required' });
  return serveSWR(res, `flow:${symbol}`, 21600_000, async () => {
...
`;
      try { return await _pyExecMedium(py); } catch(e) { console.error('earnings 실패:', symbol, e.message?.slice(0,120)); return {}; }
  });
});

// ─────────────────────────────────────────────────────────────────────────────
async function fetchPeersFromNaver(symbol, isKr) {
  try {
    const sfx = isKr ? '' : '.O';
    const d = await fetchJSON(`https://api.stock.naver.com/stock/${encodeURIComponent(symbol + sfx)}/integration`, NAVER_REF);
    if (!d || !d.industryCompareInfo) return null;

    const list = isKr ? d.industryCompareInfo.domesticStocks : d.industryCompareInfo.globalStocks;
    if (!list || !list.length) return null;

    const industry = d.industryCompareInfo.globalStocks?.[0]?.industryCodeType?.industryGroupKor || '';
    const sector = industry;

    const peers = list.map(p => {
      const ticker = p.symbolCode || p.reutersCode?.split('.')[0] || '';
      if (!ticker || ticker === symbol) return null;
      return {
        ticker,
        name: p.stockName || ticker,
        price: parseFloat(p.closePrice) || 0,
        changePct: parseFloat(p.fluctuationsRatio) || 0,
        per: null,
        forwardPer: null,
        pbr: null,
        roe: null,
        marketCap: parseFloat(p.marketValueRaw) || null,
        revenueGrowth: null,
        profitMargin: null,
        div: parseFloat(p.dividendYield) || null
      };
    }).filter(Boolean);
    ...
    console.error(`[fetchPeersFromNaver] failed for ${symbol}:`, e.message);
    return null;
  }
}
...
`;
      let raw = await _pyExecMedium(py).catch(e => { console.error('peers 실패:', symbol, e.message?.slice(0,100)); return null; });
      if (!raw || (!raw.sector && !raw.industry && !raw.peers?.length)) {
        console.log(`[api/peers] Falling back to NAVER for ${symbol}...`);
        const nvPeers = await fetchPeersFromNaver(symbol, isKr);
        if (nvPeers) raw = nvPeers;
      }
      if (!raw) throw new Error('peers 데이터 조회 실패');
...
async function fetchMacroData() {
  ...
  // 1. 네이버 해외 지수 및 선물 지표 (병목 제로)
  const naverJobs = [
    ['.INX', 'sp500', 'index', false],
    ['.IXIC', 'nasdaq', 'index', false],
    ['.DJI', 'dow', 'index', false],
    ['.VIX', 'vix', 'index', true],
    ['.N225', 'nikkei', 'index', false],
    ['.HSI', 'hangseng', 'index', false],
    ['.GDAXI', 'dax', 'index', false],
    ['GCcv1', 'gold', 'futures', false],
    ['CLcv1', 'oil', 'futures', false],
    ['SIcv1', 'silver', 'futures', false],
    ['NGcv1', 'naturalGas', 'futures', false],
    ['HGcv1', 'copper', 'futures', false],
  ];
  ...
  await Promise.allSettled(fxJobs.map(async ([code, key]) => {
    try {
      const d = await fetchJSON(`https://api.stock.naver.com/marketindex/exchange/${code}`, NAVER_REF);
      const info = d.exchangeInfo;
      const price = parseFloat(info.closePrice.replace(/,/g, ''));
      const change = parseFloat(info.fluctuationsRatio) * (info.fluctuationsType.name === 'FALLING' ? -1 : 1);
  ...
  // 3. 업비트 가상자산 지표 (레이트 리밋 안전)
  try {
    const res = await fetch('https://api.upbit.com/v1/ticker?markets=KRW-BTC,KRW-ETH');
    if (res.ok) {
      const coins = await res.json();
      coins.forEach(r => {
        const key = r.market === 'KRW-BTC' ? 'btc' : 'eth';
        result[key] = { value: r.trade_price, change: Math.round(r.signed_change_rate * 10000) / 100 };
      });
    }
  } catch {}

  // 4. 미국 국채 금리 — 네이버 금융 Bond API (야후 v8/yfinance 타임아웃 이슈 해결)
  const usNaverBondJobs = [
    ['US10YT=RR', 'us10y'],
    ['US3YT=RR',  'us3y'],
    ['US5YT=RR',  'us5y'],
  ];
  await Promise.allSettled(usNaverBondJobs.map(async ([code, key]) => {
    try {
      const d = await fetchJSON(`https://api.stock.naver.com/marketindex/bond/${code}`, NAVER_REF);
      if (d && d.closePrice) {
      ...
      console.error(`[fetchMacroData] Naver US bond fetch failed for ${code}:`, err.message);
    }
  }));



  // 4b. 네이버 금융 한국 국고채 금리 지표 (실시간 데이터 직접 연동)
  const naverBondJobs = [
    ['KR10YT=RR', 'kr10y'],
    ['KR3YT=RR', 'kr3y'],
  ];
  ...
  // 5. 미국 장단기 금리차 (10Y - 3Y Spread) 계산 및 가공
  if (result.us10y && result.us3y) {
    const spread = result.us10y.value - result.us3y.value;
    const spreadChange = (result.us10y.change || 0) - (result.us3y.change || 0);
    result['us10y3y'] = { value: parseFloat(spread.toFixed(3)), change: parseFloat(spreadChange.toFixed(3)) };
  }
...
`;
      try {
        const parsed = await _pyExecMedium(pyCode);
...
async function fetchTopByMarketCap(limit = 700) {
  return cached(`topMcap:${limit}`, 86400_000, async () => {
    ...
    const top = candidates.filter(c => c.ticker && c.ticker !== 'undefined').slice(0, limit);
...
async function precomputeTopAnalysis(N = 300) {
  ...
  try {
    ...
    // signalStore 엔트리는 'ticker' 또는 'symbol' 키 혼용 → 둘 다 지원
    let targets = [..._signalStore.values()]
      .map(s => ({ ticker: s.ticker || s.symbol, market: s.market }))
      .filter(t => t.ticker && t.ticker !== 'undefined' && t.market)
      .slice(0, N);
    if (targets.length < N) {
      const uni = await fetchTopByMarketCap(N).catch(() => []);
      const extra = uni.filter(t => t.ticker && t.ticker !== 'undefined');
      const existing = new Set(targets.map(t => t.ticker));
      targets = [...targets, ...extra.filter(t => !existing.has(t.ticker))].slice(0, N);
    }
    ...
    for (const t of targets) {
      if (!t.ticker || t.ticker === 'undefined') { skipped++; continue; }
  ...
  setTimeout(async () => {
    try {
      ...
      if (existsSync(SCREENER_CACHE_FILE)) {
        try {
          const cache = JSON.parse(readFileSync(SCREENER_CACHE_FILE, 'utf-8'));
          const ageMs = Date.now() - (cache.updatedAt || 0);
          if (ageMs < 12 * 60 * 60 * 1000) {
            needsUpdate = false;
            console.log(`  ✓ 스크리너 캐시가 신선합니다 (나이: ${Math.round(ageMs / 60000)}분). 즉시 백그라운드 갱신을 생략합니다.`);
          }
        } catch {}
      }
