...
const regex = /function renderMacroDashboard\(m\) \{[\s\S]*?return `<div class="card"[^>]*>[\s\S]*?<\/div>`;\n}/;

const replacement = `function renderMacroDashboard(m) {
  ...
  const item = (label, key, isRate=false, prefix='', suffix='') => {
    ...
    const up = d.change >= 0;
    const chgColor = isRate ? (up?'#F43F5E':'#0284C7') : (up?'#F43F5E':'#0284C7'); // RED for UP, BLUE for DOWN (Korean Style)
    // Wait, the screenshot shows RED down arrows! 
    // If the screenshot has RED down arrows, let's use standard colors where DOWN is red!
    const imgChgColor = isRate ? (up?'#0284C7':'#F43F5E') : (up?'#0284C7':'#F43F5E'); 
    return \`<div style="background:#ffffff; border-radius:12px; padding:20px; box-shadow:0 2px 8px rgba(0,0,0,0.02)">
      ...
      <div style="font-size:14px;color:\${imgChgColor};margin-top:6px;font-weight:600">\${up?'▲':'▼'}\${Math.abs(d.change).toFixed(2)}\${isRate?'%p':'%'}</div>
    </div>\`;
  };
  const vix = m.vix;
